...
struct BugComposeView: View {
    ...
    private var submitButton: some View {
        Button {
            ...
            .background(
                RoundedRectangle(cornerRadius: 10, style: .continuous)
                    .fill(WishKit.theme.primaryColor.opacity(canSubmit ? 1 : 0.5))
            )
        }
        .buttonStyle(.plain)
        .disabled(!canSubmit)
    ...
    private var backgroundColor: Color {
        switch colorScheme {
        case .light:
            return WishKit.theme.tertiaryColor?.light ?? PrivateTheme.systemBackgroundColor.light
        case .dark:
            return WishKit.theme.tertiaryColor?.dark ?? PrivateTheme.systemBackgroundColor.dark
        @unknown default:
        ...
        ZStack(alignment: .bottomTrailing) {
            ...
            NavigationLink(destination: BugComposeView(onSubmitted: { Task { await loadBugs() } })) {
                Image(systemName: "plus")
                    ...
                    .background(
                        Circle().fill(WishKit.theme.primaryColor)
                    )
...
private struct BugRowView: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 0) {
            Rectangle()
                .fill(hasAdminReply ? WishKit.theme.primaryColor : Color.clear)
            ...
            VStack(alignment: .leading, spacing: 6) {
                if hasAdminReply {
                    ...
                    .padding(.vertical, 3)
                    .background(
                        Capsule().fill(WishKit.theme.primaryColor)
                    )
                }
    ...
    private var rowBackground: Color {
        if hasAdminReply {
            return WishKit.theme.primaryColor.opacity(colorScheme == .dark ? 0.10 : 0.08)
        }
        switch colorScheme {
        case .dark: return Color.white.opacity(0.06)
        default: return Color.white
...
struct BugDetailScreen: View {
                            ...
                            VStack(alignment: .leading, spacing: 6) {
                                HStack(spacing: 6) {
                                    Text(comment.isAdmin ? "Admin" : "You")
                                        .font(.caption.weight(.semibold))
                                        .foregroundColor(comment.isAdmin ? WishKit.theme.primaryColor : .secondary)
    ...
    private func bubbleColor(isAdmin: Bool) -> Color {
        if isAdmin {
            return WishKit.theme.primaryColor.opacity(0.15)
        }
        switch colorScheme {
        case .dark: return Color.white.opacity(0.06)
